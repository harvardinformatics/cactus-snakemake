...
cactuslib_logger.debug(f"New genome file will be at : {NEW_GENOME_FILE}");
cactuslib_logger.debug(f"Parsed ancestor of sub-tree: {ANCNAME}");
...
rule all:
    input:
        os.path.join(LOG_DIR, "replace-genome.stamp")
        #os.path.join(LOG_DIR, "remove-genome.stamp")
        #os.path.join(OUTPUT_DIR, f"{ANCNAME}.hal")
        #NEW_GENOME_FILE
