...
INPUT_FILE = os.path.join(OUTPUT_DIR, "cactus-update-input.txt");
if MAIN:
...
def _getSeqFileForNode(seq_out_file, node_name):
    with open(seq_out_file, "r") as f:
        ...
        for line in f:
            ...
            if parts[0] == node_name:
                return parts[1];
    raise ValueError(f"Node '{node_name}' not found in {seq_out_file}");

NEW_GENOME_FILE, _ = UPDATELIB.getGenomesToAddReplace(CACTUS_SEQ_OUTPUT, GENOME_NAME);
# Get the genomes to add from the input file

# if MAIN:
#     with open(CACTUS_SEQ_OUTPUT) as f:
#         contents = f.read()

#     contents = re.sub(r'(\S+)\.fa\.gz\b', r'\1.fa', contents)

#     with open(CACTUS_SEQ_OUTPUT, "w") as f:
#         f.write(contents)
# Introduced in cactus 3.0.0, cactus-update-prepare compresses fasta files in the seq_file.out
# This breaks downstream rules since now some nodes have .fa and some have .fa.gz, so we specify only uncompressed files in the seq_file.out

ORIG_BL = UPDATELIB.getOrigBranchLength(CACTUS_SEQ_OUTPUT, CHILD, ANCNAME, TOP_BL);

ANC_SEQ_FILE = _getSeqFileForNode(CACTUS_SEQ_OUTPUT, ANCNAME);
ANC_SEQ_BASE = os.path.basename(ANC_SEQ_FILE);
ANC_SEQ_PATH = os.path.join(OUTPUT_DIR, ANC_SEQ_BASE);
...
rounds = { ANCNAME : { "name" : ANCNAME, 
                        ...
                        "convert-output" : ANC_SEQ_PATH },
            PARENT : { "name" : PARENT, 
                        "blast-input" : [os.path.join(OUTPUT_DIR, ANCNAME + ".hal"), ANC_SEQ_PATH], 
...
rule preprocess:
    ...
    log:
        job_log = os.path.join(LOG_DIR, f"{GENOME_NAME}.preprocess.log")
    resources:
...
## This rule runs cactus-preprocess for the new genome to be added

####################

rule blast_new_genome:
    input:
        NEW_GENOME_FILE
    output:
        paf_file = rounds[ANCNAME]['blast-output']
    params:
...
## This rule runs cactus-blast for the new genome

####################

rule align_new_genome:
    input:
        paf_file = rounds[ANCNAME]['blast-output']
    output:
        hal_file = rounds[ANCNAME]['align-output']
    params:
        path = CACTUS_PATH_TMP,
    ...
    run:
        ...
        CACTUSLIB.runCommand(cmd, params.host_tmp_dir, log.job_log, params.rule_name, params.node)
## This rule runs cactus-align for the new genome

####################

rule convert_new_genome:
    input:
        hal_file = rounds[ANCNAME]['align-output']
    output:
        fa_file = rounds[ANCNAME]['convert-output']
    params:
        path = CACTUS_PATH,
        node = ANCNAME,
        job_tmp_dir = os.path.join("/tmp", f"{ANCNAME}-convert"), # This is the tmp dir in the container, which is bound to the host tmp dir
        host_tmp_dir = os.path.join(TMPDIR, f"{ANCNAME}-convert"), # This is the tmp dir for the host system, which is bound to /tmp in the singularity container        
        rule_name = "convert"
    log:
        job_log = os.path.join(LOG_DIR, f"{ANCNAME}.convert.log")
    resources:
    ...
    run:
        ...
        CACTUSLIB.runCommand(cmd, params.host_tmp_dir, log.job_log, params.rule_name, params.node)
## This rule runs hal2fasta to convert the .hal file for the new internal node to .fasta

####################

rule blast_parent:
    input:
        rounds[PARENT]['blast-input']
    output:
        paf_file = rounds[PARENT]['blast-output']
    params:
        path = CACTUS_PATH_TMP,
        seq_out = CACTUS_SEQ_OUTPUT,
        node = PARENT,
        job_tmp_dir = os.path.join("/tmp", f"{PARENT}-blast"), # This is the tmp dir in the container, which is bound to the host tmp dir
        host_tmp_dir = os.path.join(TMPDIR, f"{PARENT}-blast"), # This is the tmp dir for the host system, which is bound to /tmp in the singularity container
        gpu_opt = USE_GPU,
        gpu_num = config['rule_resources']['blast']['gpus'],
        rule_name = "blast"
    log:
        job_log = os.path.join(LOG_DIR, f"{PARENT}.blast.log")
    resources:
        **getRuleResources("blast"),
        slurm_extra = "--gpus-per-task=" + str(config['rule_resources']['blast']['gpus']) if USE_GPU else "",
        tasks_per_gpu = 0
    run:
        ...
        CACTUSLIB.runCommand(cmd, params.host_tmp_dir, log.job_log, params.rule_name, params.node)
## This rule runs cactus-blast for the parent node of the new ancestral node
...
rule align_parent:
    input:
        paf_file = rounds[PARENT]['blast-output']
    output:
        hal_file = rounds[PARENT]['align-output']
    params:
        path = CACTUS_PATH_TMP,
        cactus_file = CACTUS_SEQ_OUTPUT,
        node = PARENT,
        keg_patch_file = KEG_PATCH_FILE,
        job_tmp_dir = os.path.join("/tmp", f"{PARENT}-align"), # This is the tmp dir in the container, which is bound to the host tmp dir
        host_tmp_dir = os.path.join(TMPDIR, f"{PARENT}-align"), # This is the tmp dir for the host system, which is bound to /tmp in the singularity container
        work_dir = TMPDIR,
        rule_name = "align"
    log:
        job_log = os.path.join(LOG_DIR, f"{PARENT}.align.log")
    resources:
        **getRuleResources("align")
    run:
        cmd = params.path + [
...
rule append_subtrees:
    input:
        hal_file = rounds[ANCNAME]['align-output'],
        parent_hal_file = rounds[PARENT]['align-output']
    output:
        stamp = os.path.join(LOG_DIR, "append-subtrees.stamp")
    params:
        path = CACTUS_PATH_TMP,
        cactus_file = CACTUS_SEQ_OUTPUT,
        node = PARENT,
        job_tmp_dir = os.path.join("/tmp", f"{PARENT}-append-subtrees"), # This is the tmp dir in the container, which is bound to the host tmp dir
        host_tmp_dir = os.path.join(TMPDIR, f"{PARENT}-append-subtrees"), # This is the tmp dir for the host system, which is bound to /tmp in the singularity container
        work_dir = TMPDIR,
        rule_name = "append_subtrees"
    log:
        job_log = os.path.join(LOG_DIR, f"{PARENT}.append-subtrees.log")
    resources:
        **getRuleResources("append_subtrees")
    run:
        cmd = params.path + [
            "cactus-halAppendSubtrees",
            params.job_tmp_dir,
            input.parent_hal_file,
            input.hal_file,
            input.parent_hal_file,
            "--logInfo",
            "--retryCount", "0",
...
rule add_to_branch:
    input:
        ...
        append_subtrees_stamp = os.path.join(LOG_DIR, "append-subtrees.stamp")
    output:
