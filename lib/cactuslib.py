...
def getExecutor(args, config):
    ...
    looking for either --executor or -e.
    """

    for i, arg in enumerate(args):
        if arg in ("--executor", "-e"):
            # Look ahead to next argument
            try:
                return args[i + 1]
            except IndexError:
                return "unknown"  # flag present, but no value
        elif arg.startswith("--executor="):
            return arg.split("=", 1)[1]
        elif arg.startswith("-e="):
            return arg.split("=", 1)[1]

    return config.get("executor", "none")  # if no flag found, check the configs
...
def pipelineSetup(config, args, version_flag, info_flag, config_flag, debug, workflow, pad=50):
    ...
    if main_flag:
        ...
        top_level_executor = open(".snakemake/top-level-executor.txt", "r").read().strip();
    ...
    if config_flag or debug:
        ...
        cactuslib_logger.debug(spacedOut("Specfied executor", debug_pad) + top_level_executor);
    ...
    return main_flag, dry_run_flag, output_dir, log_dir, tmp_dir, log_level, log_verbosity, top_level_executor
...
# Return dict of all requested resource keys for a rule (with fallback to defaults).
    ...
    rule_resources = {
        slurm_resource_map[resource] : getResource(config, top_level_executor, rule_name, resource)
        for resource in keys
    }

    # for key, value in rule_resources.items():
    #     cactus_logger.info(f"Rule {rule_name} resource '{key}' set to {value}");
